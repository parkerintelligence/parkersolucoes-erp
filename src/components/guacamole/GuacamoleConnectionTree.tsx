...
export const GuacamoleConnectionTree = ({
  ...
  const organizedConnections = () => {
    ...
    return Object.values(groups).filter(group => 
      group.connections.length > 0 || group.identifier === 'general'
    );
  };
  ...
  return (
    <div className="space-y-2">
      {groups.map(group => (
        <Card key={group.identifier} className="bg-slate-800 border-slate-700">
          <div className="p-3">
            <Button
              variant="ghost"
              className="w-full justify-start p-0 h-auto hover:bg-slate-700"
              onClick={() => toggleGroup(group.identifier)}
            >
              <div className="flex items-center gap-2 text-left">
                {expandedGroups.has(group.identifier) ? (
            ...
            {expandedGroups.has(group.identifier) && (
              <div className="mt-3 space-y-2">
                {group.connections.length === 0 ? (
                  <div className="ml-6 py-4 text-center text-slate-500 text-sm">
                    Nenhuma conexão neste grupo
                  </div>
                ) : (
                  group.connections.map(connection => (
                    <Card key={connection.identifier} className="ml-6 bg-slate-700 border-slate-600">
                          ...
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <Badge 
                                  variant="outline" 
                                  className={`text-xs ${getProtocolColor(connection.protocol)}`}
                                >
                                  {connection.protocol.toUpperCase()}
                                </Badge>
                                <Badge 
                                  variant="secondary" 
                                  className={`text-xs ${getStatusColor(getConnectionStatus(connection))}`}
                                >
