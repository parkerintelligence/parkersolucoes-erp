      ...
      <Card className="bg-card border-border">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Dialog open={isCreating} onOpenChange={setIsCreating}>
                ...
                <DialogHeader>
                  <DialogTitle className="text-foreground">Criar Webhook</DialogTitle>
                ...
                <div className="space-y-4">
                  <div>
                    <Label className="text-muted-foreground">Nome do Webhook</Label>
                    <Input value={newWebhook.name} onChange={e => setNewWebhook(prev => ({
                      ...prev,
                      name: e.target.value
                    }))} placeholder="Ex: Alertas Críticos" className="bg-secondary border-border text-foreground" />
                  </div>
                  ...
                  <div>
                    ...
                    <Select value={newWebhook.trigger_type} onValueChange={(value: any) => setNewWebhook(prev => ({
                      ...
                      <SelectContent className="bg-card border-border">
                        {Object.entries(triggerTypeLabels).map(([value, label]) => <SelectItem key={value} value={value} className="text-foreground">
                            {label}
                          </SelectItem>)}
                      </SelectContent>
                    </Select>
                  ...
                  <div>
                    <Label className="text-muted-foreground">Número do WhatsApp</Label>
                  ...
                  <div>
                    ...
                    }))} placeholder="🚨 Alerta Zabbix: {problem_name} no host {host_name}" className="bg-secondary border-border text-foreground" rows={3} />
                  ...
                  <div className="flex gap-2 pt-4">
                    <Button onClick={handleCreateWebhook} className="bg-blue-600 hover:bg-blue-700 text-foreground" disabled={createWebhook.isPending}>
                      {createWebhook.isPending ? 'Criando...' : 'Criar'}
                    </Button>
                    <Button variant="outline" onClick={() => setIsCreating(false)} className="border-border text-muted-foreground hover:bg-secondary">
                      Cancelar
                    </Button>
                  </div>
                </div>
              ...
              <Button onClick={simulateZabbixIncidents} variant="outline" className="bg-orange-600 hover:bg-orange-700 border-orange-500 text-foreground" disabled={isSimulating}>
        ...
        <CardContent>
          {webhooks.length === 0 ? <div className="text-center py-8">
            ...
            return <div key={webhook.id} className="p-4 bg-secondary/50 rounded-lg border border-border">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="outline" size="sm" onClick={() => setEditingWebhook(webhook)} className="bg-muted border-border text-foreground hover:bg-slate-500">
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => handleDeleteWebhook(webhook.id)} className="bg-red-600 border-red-500 text-foreground hover:bg-red-500">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <br />
                        <span className="text-muted-foreground font-mono">{actions?.whatsapp_number || 'N/A'}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Execuções:</span>
                        <br />
                        <span className="text-muted-foreground">{webhook.trigger_count}</span>
                      </div>
                    </div>
                    {actions?.custom_message && <div className="mt-3 p-2 bg-card rounded text-sm">
                        <span className="text-muted-foreground">Mensagem:</span>
                        <br />
                        <span className="text-muted-foreground">{actions.custom_message}</span>
                      </div>}
                  </div>;
          })}
            </div>}
      ...
      <Dialog open={!!editingWebhook} onOpenChange={open => !open && setEditingWebhook(null)}>
        <DialogContent className="bg-card border-border text-foreground max-w-lg">
          ...
          {editingWebhook && <div className="space-y-4">
              ...
              ...prev,
              name: e.target.value
            } : null)} className="bg-secondary border-border text-foreground" />
              </div>
            ...
            } : null)} className="bg-secondary border-border text-foreground" rows={3} />
              </div>

              <div className="flex gap-2 pt-4">
                <Button onClick={handleUpdateWebhook} className="bg-blue-600 hover:bg-blue-700 text-foreground" disabled={updateWebhook.isPending}>
                  {updateWebhook.isPending ? 'Salvando...' : 'Salvar'}
                </Button>
                <Button variant="outline" onClick={() => setEditingWebhook(null)} className="border-border text-muted-foreground hover:bg-secondary">
                  Cancelar
                </Button>
              </div>
            </div>}
