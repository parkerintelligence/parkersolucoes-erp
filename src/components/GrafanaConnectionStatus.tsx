      ...
      <Card className="border-orange-600 bg-orange-900/20">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-orange-300 mb-4">
            Configure API Token ou usuário/senha do Grafana no painel de administração.
          </p>
          <div className="text-sm text-orange-400 mb-4">
            <p>Servidor: {grafanaIntegration.base_url}</p>
            <p>Nome: {grafanaIntegration.name}</p>
          </div>
          <Button 
            onClick={() => window.location.href = '/admin'}
      ...
      <Card className="border-blue-600 bg-blue-900/20">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-sm text-blue-400 mt-2">
            Usando: {hasApiToken ? 'API Token' : `usuário "${grafanaIntegration.username}"`}
          </p>
        </CardContent>
      </Card>
    );
      ...
      <Card className="border-red-600 bg-red-900/20">
        <CardContent className="p-6 text-center">
          ...
          <div className="text-sm text-red-400 mb-4">
            <p>Servidor: {grafanaIntegration.base_url}</p>
            <p>Método: {hasApiToken ? 'API Token' : `Basic Auth (${grafanaIntegration.username})`}</p>
          </div>
        ...
        <CardContent className="p-6 text-center">
          ...
          <div className="text-green-300">
            ...
            <p className="text-sm text-green-400 mt-1">
              Autenticado via: {hasApiToken ? 'API Token' : `usuário "${grafanaIntegration.username}"`}
            </p>
          </div>
        </CardContent>
      </Card>
