          ...
          <DialogContent className="bg-card border-border">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Mínimo 6 caracteres" className="bg-secondary border-border text-foreground" />
              </div>
              <div>
                ...
                <Select value={newRole} onValueChange={setNewRole}>
                  ...
                  <SelectContent className="bg-secondary border-border">
                    <SelectItem value="user" className="text-foreground">Usuário</SelectItem>
                    <SelectItem value="admin" className="text-foreground">Admin</SelectItem>
                    <SelectItem value="master" className="text-foreground">Master</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowCreateDialog(false)} className="border-border text-muted-foreground">Cancelar</Button>
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="space-y-3">
            {users.map((user) => (
              <div key={user.id} className="flex items-center justify-between p-4 bg-secondary/50 rounded-lg border border-border">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Dialog open={resetUserId === user.id} onOpenChange={(open) => { if (!open) { setResetUserId(null); setResetPassword(''); } }}>
                    <DialogTrigger asChild>
                      <Button variant="outline" size="sm" onClick={() => setResetUserId(user.id)} className="border-border text-muted-foreground hover:bg-muted">
                        <KeyRound className="h-4 w-4" />
                      </Button>
                    </DialogTrigger>
                    <DialogContent className="bg-card border-border">
                      <DialogHeader>
                        <DialogTitle className="text-foreground">Resetar Senha - {user.email}</DialogTitle>
                      </DialogHeader>
                      <div>
                        <Label className="text-muted-foreground">Nova Senha</Label>
                        <Input type="password" value={resetPassword} onChange={(e) => setResetPassword(e.target.value)} placeholder="Nova senha" className="bg-secondary border-border text-foreground" />
                      </div>
                      <DialogFooter>
