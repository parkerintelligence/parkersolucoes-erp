...
-- Atualizar a função calculate_next_execution para calcular corretamente a próxima execução
  ...
  IF target_days != '*' THEN
    ...
    ELSIF target_days ~ '^[0-6](,[0-6])*$' THEN
      ...
      WHILE NOT (EXTRACT(DOW FROM next_exec)::INTEGER = ANY(target_day_array)) LOOP
        ...
        IF next_exec > from_time + INTERVAL '7 days' THEN
          EXIT;
        END IF;
      END LOOP;
...
-- Atualizar todas as próximas execuções existentes
