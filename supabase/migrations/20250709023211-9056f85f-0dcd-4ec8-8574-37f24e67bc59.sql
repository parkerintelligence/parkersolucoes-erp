...
-- Corrigir a lógica da função calculate_next_execution para calcular corretamente a próxima execução
...
DECLARE
  ...
  brasilia_next_exec TIMESTAMP WITH TIME ZONE;
BEGIN
  ...
  brasilia_next_exec := date_trunc('day', brasilia_time) + 
                        INTERVAL '1 hour' * target_hour + 
                        INTERVAL '1 minute' * target_minute;
  
  -- Se já passou o horário hoje, começar de amanhã
  IF brasilia_next_exec <= brasilia_time THEN
    brasilia_next_exec := brasilia_next_exec + INTERVAL '1 day';
  END IF;
  ...
  IF target_days != '*' THEN
    ...
    IF target_days = '1-5' THEN
      ...
      WHILE EXTRACT(DOW FROM brasilia_next_exec) NOT IN (1,2,3,4,5) LOOP
        brasilia_next_exec := brasilia_next_exec + INTERVAL '1 day';
      END LOOP;
    ...
    ELSIF target_days ~ '^[0-6](,[0-6])*$' THEN
      ...
      WHILE NOT (EXTRACT(DOW FROM brasilia_next_exec)::INTEGER = ANY(target_day_array)) LOOP
        brasilia_next_exec := brasilia_next_exec + INTERVAL '1 day';
        -- Evitar loop infinito
        IF brasilia_next_exec > brasilia_time + INTERVAL '7 days' THEN
          EXIT;
        END IF;
      END LOOP;
    -- Para um único dia específico
    ELSIF target_days ~ '^\d$' THEN
      current_day := EXTRACT(DOW FROM brasilia_next_exec)::INTEGER;
  ...
  -- Converter de volta para UTC mantendo o mesmo momento no tempo
  -- Usar timezone('UTC', brasilia_next_exec AT TIME ZONE 'America/Sao_Paulo') 
  -- para garantir que tratamos o timestamp como sendo no timezone de Brasília
  RETURN timezone('UTC', brasilia_next_exec AT TIME ZONE 'America/Sao_Paulo');
END;
$function$;
